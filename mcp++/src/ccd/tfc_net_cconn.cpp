...
// NULL
ConnCache* CConnSet::AddConn(int fd, unsigned long long &flow, bool purge) {
	static unsigned long long i_flow = (unsigned)time(0);
	
	ConnCache* cc = NULL;
	if(list_empty(&_free_ccs)) {
		if(purge) {
			cc = list_entry(_used_ccs.next, struct ConnCache, _next);
			if ( is_ccd ) {
				CloseCC(cc, ccd_rsp_disconnect);
			} else {
				CloseCC(cc, dcc_rsp_disconnected);
	...
	if(_flow_2_cc[flow % _flow_slot_size] != NULL) {
		unsigned last_free_slot = cc->_flow % _flow_slot_size;
		if(_flow_2_cc[last_free_slot] == NULL) {
			unsigned this_slot = flow % _flow_slot_size;
			if(this_slot != last_free_slot) {
				if(this_slot < last_free_slot)
					flow += last_free_slot - this_slot;
				else
					flow += _flow_slot_size - this_slot + last_free_slot;		
				//尽量保持flow是单调递增的，除非已经达到最大值回滚	
				if(flow < SOCK_FLOW_MIN)
					flow += _flow_slot_size;
				else if(flow >= SOCK_FLOW_MAX)
					flow = _flow_slot_size + last_free_slot;
			}
				
			i_flow = flow;
			_flow_2_cc[last_free_slot] = cc;
		}
		else {														
			fprintf(stderr, "CCD: no free conncache mapping slot, %llu,%u\n", flow, _flow_slot_size);
#ifdef _OUTPUT_LOG
			if ( output_log ) {
				syslog(LOG_USER | LOG_CRIT | LOG_PID, "MCP++ CCD: No free conncache mapping slot in AddConn!\n");
			}
#endif
			return NULL;
		}
...
ConnCache* CConnSet::AddConnDCC(int fd, unsigned long long flow, bool purge) {
	...
	if(list_empty(&_free_ccs)) {
		if(purge) {
			...
			fprintf(stderr, "no free conncache, fd - %d, _max_conn - %u, flow(dcc ignore this) - %llu\n",
				fd, _max_conn, flow);
#ifdef _OUTPUT_LOG
			if ( output_log ) {
				syslog(LOG_USER | LOG_CRIT | LOG_PID, "MCP++ CCD: No free connection in AddConn!\n");
	...
	if(_flow_2_cc[flow % _flow_slot_size] != NULL) {
		// fprintf(stderr, "hash conflict, flow=%llu\n", flow);
		ConnCache* prev_cc = _flow_2_cc[flow % _flow_slot_size];
		while((prev_cc != NULL) && (prev_cc->_dcc_next_cc != NULL))
			prev_cc = prev_cc->_dcc_next_cc;
		if(prev_cc)
			prev_cc->_dcc_next_cc = cc;
	}
	else {
		_flow_2_cc[flow % _flow_slot_size] = cc;
	}
...
void CConnSet::CloseCC(ConnCache* cc, unsigned short event) {
	...
	if(is_ccd) {
		_flow_2_cc[cc->_flow % _flow_slot_size] = NULL;
	}
	else {
		ConnCache* prev_cc = _flow_2_cc[cc->_flow % _flow_slot_size];
		if(prev_cc == cc) {
			_flow_2_cc[cc->_flow % _flow_slot_size] = cc->_dcc_next_cc;
		}
		else{
			while((prev_cc != NULL) && (prev_cc->_dcc_next_cc != cc))
				prev_cc = prev_cc->_dcc_next_cc;
			if(prev_cc)
				prev_cc->_dcc_next_cc = cc->_dcc_next_cc;
		}
		cc->_dcc_next_cc = NULL;
	}
...
//void CConnSet::CloseCC(unsigned flow) {
//	ConnCache* cc = GetConnCache(flow);
//	if(!cc)
//		return;
//	
//	CloseCC(cc);
...
// 0, delay close
// 1, closed
//int CConnSet::TryCloseCC(unsigned flow) {
//	ConnCache* cc = GetConnCache(flow);
//	if(!cc)
//		return 1;
//	
//	return TryCloseCC(cc);
//}
