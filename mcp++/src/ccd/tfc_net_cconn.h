...
namespace tfc{namespace net
{
	...
	public:
		...
		ConnCache* AddConn(int fd, unsigned long long &flow, bool purge = false);
		ConnCache* AddConnDCC(int fd, unsigned long long flow, bool purge = true);
		...
		//int TryCloseCC(unsigned flow);
		void CloseCC(ConnCache* cc, unsigned short event);
		//void CloseCC(unsigned flow);
		...
		inline ConnCache* GetConnCache(unsigned long long flow) {
			...
			if(cc != NULL && cc->_flow == flow) {
				return cc;
			}	
			else {
				if ( cc != NULL ) {
					fprintf(stderr, "cc->flow %llu, flow %llu\n", cc->_flow, flow);
				}
				return NULL;
			}
		}
		inline ConnCache* GetConnCacheDCC(unsigned long long flow) {
			ConnCache* cc = _flow_2_cc[flow % _flow_slot_size];
			while(cc != NULL) {
				if(cc->_flow == flow) 
					return cc;
